package me.koba1.bedwars.commands;

import me.koba1.bedwars.commands.argumentMatchers.ArgumentMatcher;
...
public abstract class MainCommand implements TabExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (sender.hasPermission(subCommand.getPermission())) {
            try {
                ...
                //Bukkit.getLogger().severe("Az event fileok között argumentumok hiányoznak!");
                subCommand.usage(sender);
            }
        }
        else
            sender.sendMessage(Messages.NO_PERMISSION.language(sender).queue());
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 1) {
            List<String> subCommandsTC = subCommands
                    .stream()
        ...
        List<String> subCommandTB = subCommand.getTabCompletion(args.length - 2, Arrays.copyOfRange(args, 1, args.length));
